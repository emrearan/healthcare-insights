...
class SourceBTransformer(BaseTransformer):
    ...
    col("visit.provider_notes.author_test").alias("provider_notes_author") # Different from Source A
    ).withColumn("source", lit(self.source))
    ...
    def transform_treatments(self) -> DataFrame:
        ...
        result = df.select(
            ...
            col("visit_id"),
            col("diagnosis_code"),
            col("treatment.drug_test").alias("treatments_drug"), # Different from Source A
            col("treatment.dose_test").alias("treatments_dose") # Different from Source A
        )
