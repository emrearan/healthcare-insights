...
class SourceBTransformer(BaseTransformer):
    def __init__(self, spark, date, source):
        super().__init__(spark, date, source)
        self.raw_df = self.spark.read.option("multiline", "true").json(
            f"input_data/{source}/{date}/visit_events.json"
        )
    ...
    def transform_patients(self) -> DataFrame:
        ...
        latest_data = test_df.where(col("source") == lit(self.source)).where(
            col("snapshot_date") == latest_day
        )

        df: DataFrame = self.raw_df
        new_data = (
            df.select("patient_id", "name")
        ...
        combined_df = latest_data.unionByName(new_data).withColumn(
            "snapshot_date", to_date(lit(self.date), "yyyyMMdd")
        )
    ...
    def transform_visits(self) -> DataFrame:
        ...
        result = (
            df.select(col("patient_id"), explode("visits").alias("visit"))
            .select(
                col("patient_id"),
                ...
                col("visit.provider_notes.author_test").alias(
                    "provider_notes_author"
                ),  # Different from Source A
            )
            .withColumn("source", lit(self.source))
        )
    ...
    def transform_treatments(self) -> DataFrame:
        ...
        result = (
            df.select(explode("visits").alias("visit"))
            .select(
                col("visit.date").alias("visit_date"),
                col("visit.visit_id").alias("visit_id"),
                explode("visit.diagnoses").alias("diagnosis"),
            ...
            .select(
                ...
                col("visit_id"),
                col("diagnosis_code"),
                col("treatment.drug_test").alias(
                    "treatments_drug"
                ),  # Different from Source A
                col("treatment.dose_test").alias(
                    "treatments_dose"
                ),  # Different from Source A
            )
        )
